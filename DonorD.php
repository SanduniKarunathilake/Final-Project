...
if (isset($_POST['btnUpdate'])) {
    ...
    $sql = "UPDATE tbl_sponsor_donor 
            SET Name = '$name', Age = '$age', Address = '$address', Email = '$email', pwd = '$password',  TeleNum = '$contact',  Type = '$type'
            WHERE SDID = '$sdid'";
    ...
    $sql = "INSERT INTO tbl_sponsor_donor  (Name, SDID, Address, Email, Age,  Type, TeleNum, pwd) 
            VALUES ('$name', '$sdid', '$address', '$email', '$age',  '$type', '$contact', '$password')";
    
    $result = mysqli_query($con, $sql);

    if ($result) {
        showMessage("Registration Successful", $sdid, $password);
    } else {
...
function showMessage($message, $sdid = '', $password = '') {
    echo '<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
    ...
    if ($sdid != '' && $password != '') {
        echo '<h2>Your Login Details:</h2>
              <p><strong>Username:</strong> ' . $sdid . '</p>   
